...
const nextReleaseOffset = lines.slice(latestReleaseIndex + 1).findIndex((x: string) => x.startsWith("### "))
const releaseNotes = lines.slice(latestReleaseIndex + 1, nextReleaseOffset >= 0 ? nextReleaseOffset + latestReleaseIndex + 1 : undefined).join("\n").trim();
