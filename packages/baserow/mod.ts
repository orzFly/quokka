...
export type BaserowFilterGroup<F extends number | string> =
  | {
    filter_type: "AND";
    filters?: BaserowFilter<F>[];
    groups?: BaserowFilterGroup<F>[];
  }
  | {
    filter_type: "OR";
    filters?: BaserowFilter<F>[];
...
export class BaserowTable<T extends Record<string, any>> {
  ...
  private convertFilter(
    filter: BaserowFilter<Extract<keyof T, string>>,
  ): BaserowFilter<number> {
  ...
  private convertFilterGroup(
    group: BaserowFilterGroup<Extract<keyof T, string>>,
  ): BaserowFilterGroup<number> {
    return {
      ...group,
  ...
  public async list(
      ...
      ...(options.filters || options.filterGroups
        ? {
          filters: JSON.stringify({
            ...
            groups: (options.filterGroups ?? []).map((g) =>
              this.convertFilterGroup(g)
            ),
          }),
        }
        : {}),
      ...(options.join
        ? Object.fromEntries(
          options.join.map((j) => [
            ...
            j.target.fields.map((f) => `field_${j.target.table.fieldMap[f]}`)
              .join(","),
          ]),
        )
        : {}),
    ...
    const response = await fetch(
      `${this.instance.backend}/database/rows/table/${this.tableId}/?${new URLSearchParams(
        params,
      )}`,
      {
  ...
  public async get(id: number): Promise<BaserowRecord<T>> {
    const response = await fetch(
      `${this.instance.backend}/database/rows/table/${this.tableId}/${id}/`,
      {
        headers: this.instance.header,
      },
    );
    ...
    const body = JSON.stringify(
      Object.fromEntries(
        Object.entries(record).map((
          [key, value],
        ) => [`field_${this.fieldMap[key as keyof T]}`, value]),
      ),
    );
    const response = await fetch(
      `${this.instance.backend}/database/rows/table/${this.tableId}/`,
      {
        method: "POST",
        headers: {
