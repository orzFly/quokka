...
function junkWord() {
  return [
    randomItem(
      "JavaScript EMCAScript CoffeeScript IcedScript TypeScript Flow ActionScript PHP"
        .split(" "),
    ),
    randomItem(
      "是 不是 可能是 大概是 应该是 一定是 可能不是 大概不是 应该不是 一定不是"
        .split(" "),
    ),
...
async function main() {
  const marline = new Marline({
    ...
    render: (marline, width) => {
      ...
      marline.bottom.set(
        0,
        `[${new Array(fillWidth).fill("=").join("")}${
          new Array(barWidth - fillWidth).fill(" ").join("")
        }] ${progress}`,
      );
